...
public:
    ...
    void declare_variable(char *type, char *name)
    {
        ...
        { //pt fiecare tip se creeaza un nou obiect de tip Data
            data = new Data(0);
        }
    ...
    {
        if (strcmp(scope_memory[name]->type, data->type) == 0)
        { //Verifica daca tipul variabilei se potriveste cu tipul valorii introduse
    ...
    {
        if (scope_memory.find(std::string(name)) != scope_memory.end())
        { //Verifaca daca exista variabila in memoria din Scope
            return true;
        }
        return false;
    }
    ...
    int tableprint()
    { // printeaza tipul, id-ul si valoarea simbolurilor in symbol_table.txt
        std::ofstream myfile;
        myfile.open("symbol_table.txt", std::ofstream::app);
        for (auto i : scope_memory)
        {
